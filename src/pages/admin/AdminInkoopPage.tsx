...
import InkoopverklaringenTab from '@/components/admin/inkoop/InkoopverklaringenTab';
...
const pageTabs = [
  ...
  { label: "Inkoopverklaringen", value: "verklaringen" },
];

export default function AdminInkoopPage() {
  const [pageTab, setPageTab] = useState("shortlist");
    ...
    <div className="space-y-4 md:space-y-5 max-w-full">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground text-xs md:text-sm mt-0.5">Potentiële aankopen en inkoopverklaringen</p>
        </div>
        {pageTab === "shortlist" && (
      ...
      {/* Page Tabs */}
      <SlidingTabs tabs={pageTabs} value={pageTab} onChange={setPageTab} />

      {pageTab === "verklaringen" ? (
        <InkoopverklaringenTab />
      ) : (
        <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-secondary/30">
                    ...
                    <th className="text-left px-2 py-2 text-[11px] font-medium text-muted-foreground w-8"></th>
                    <th className="text-right px-3 py-2 text-[11px] font-medium text-muted-foreground">Vraagprijs</th>
                    <th className="text-right px-3 py-2 text-[11px] font-medium text-muted-foreground">Totaal inkoop</th>
                    <th className="text-right px-3 py-2 text-[11px] font-medium text-muted-foreground">Verkoop</th>
                    <th className="text-right px-3 py-2 text-[11px] font-medium text-muted-foreground">Winst</th>
                    <th className="text-right px-3 py-2 text-[11px] font-medium text-muted-foreground">Marge</th>
                        ...
                        <td className="px-3 py-1.5">
                          <div className="flex items-center gap-2">
                            <div>
                              ...
                              <p className="text-[11px] text-muted-foreground">{c.bouwjaar} · {c.kilometerstand.toLocaleString('nl-NL')} km</p>
                            </div>
                            {isImport && <span className={cn(BADGE_BASE, 'bg-sky-500/15 text-sky-400 border-sky-500/30 uppercase tracking-wider')}>Import</span>}
                            {isVeiling && <span className={cn(BADGE_BASE, 'bg-orange-500/15 text-orange-400 border-orange-500/30 uppercase tracking-wider')}>Veiling</span>}
                          </div>
                        </td>
                        <td className="px-2 py-1.5">
                          {c.bronLink && (
                            <a href={c.bronLink} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="inline-flex items-center justify-center w-6 h-6 rounded-md text-foreground hover:bg-foreground/10 border border-transparent hover:border-foreground/20 transition-all duration-150" title="Open advertentie">
